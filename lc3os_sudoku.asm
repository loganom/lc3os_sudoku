...
OS_SAVE_R6      .BLKW 1
OS_SAVE_R7      .BLKW 1
OS_SAVE_R7_OUT  .BLKW 1
OS_OUT_SAVE_R1  .BLKW 1
OS_IN_SAVE_R7   .BLKW 1
                	
MASK_HI         	.FILL x7FFF
HIGH_8_BITS 	.FILL xFF00
LOW_8_BITS      	.FILL x00FF
MASK_HI_12_BITS 	.FILL x000F
NUM_CVRT		.FILL x0030
TIM_INIT        	.FILL #40
...
TRAP_R_SHIFT
	...
	ADD R5,R4,#-2 		; default is -2 (move 1 bit)

R_SHIFT_VALUE_CALC
	ADD R1,R1,#-1		; decrement R1 (bits to move)
...
R_SHIFT_CONTINUE 	
	ADD R0,R0,R5
	BRn R_SHIFT_COMPLETE	; do work
	ADD R3,R3,#1 		; the accumulator
	BRnzp R_SHIFT_CONTINUE 	; still more to do? go back
...
TRAP_NEXT_SET_BIT
	...
	TRAP x31 		; TRAP_R_SHIFT. output in R0.

NEXT_SET_BIT_SEARCH
	AND R1,R0,x01 		; clear bits [15:1]
	BRp NEXT_SET_BIT_FOUND	; the bit is set
	ADD R5,R5,#1
	ADD R1,R1,#1 		; will always = #1
	TRAP x31 		; TRAP_R_SHIFT
	BRnzp NEXT_SET_BIT_SEARCH
